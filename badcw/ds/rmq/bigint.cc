...
void         init(int _L) {
            L = _L, initialized = 1;
            for (int i = 0; i < (1 << L); ++i) brev[i] = (brev[i >> 1] >> 1) | ((i & 1) << (L - 1));
    for (int i = 0; i < (1 << L); ++i) {
...
void fft(complex a[], int lgn, int flag) {
    ...
    if (flag == -1)
        for (int i = 0; i < n; ++i) a[i] = {a[i].a / n, a[i].b / n};
}

void fft_two_seq(complex a[], complex b[], int lgn, int flag) {
    int n = 1 << lgn;

    for (int i = 0; i < n; ++i) a[i].b = b[i].a;
...
void mul(int a[], int n, int b[], int m, int res[], int mod, int* res_len) {
    ...
    if (n < 100 / (m + 1) || n < 3 || m < 3)
        for (int i = 0; i <= n; ++i)
            for (int j = 0; j <= m; ++j) {
                ...
                res[i + j] = 0;
                int offset = 0;
                while (x >= mod) res[i + j + offset] += (x % mod), x /= mod, ++offset;
                res[i + j + offset] += x;
    ...
    // FFT
        ...
        while ((1 << lgk) <= len) ++lgk, k <<= 1;
        for (int i = 0; i <= n; ++i) com_a[i].a = a[i], com_a[i].b = 0.0;
        for (int i = 0; i <= m; ++i) com_b[i].a = b[i], com_b[i].b = 0.0;
        // multiple_case
        memset(com_a + (n + 1), 0, sizeof(com_a[0]) * (k - n - 1));
        memset(com_b + (m + 1), 0, sizeof(com_b[0]) * (k - m - 1));

        fft_two_seq(com_a, com_b, lgk, 1);
        for (int i = 0; i < k; ++i) com_a[i] = com_a[i] * com_b[i];
        fft(com_a, lgk, -1);
        for (int i = 0; i <= n + m; ++i) {
            ll x       = 1ll * res[i] + (ll)(com_a[i].a + 0.5);
            res[i]     = 0;
            int offset = 0;
            while (x >= mod) res[i + offset] += (x % mod), x /= mod, ++offset;
            res[i + offset] += x;
...
struct BigNumber {
    ...
    void input() {
        static char s[200010];  // the length of the input string is adjustable
        scanf("%s", s);
        (*this) = BigNumber(s);
    }
    bool is_zero() const {
    ...
    int print_len() const {
        ...
        while (digit[add] < tmp && add < MODD) ++add;
        return ret + add;
    }
    ...
    int compare(const BigNumber& p) const {
        if (len == p.len) {
            for (int i = len - 1; i >= 0; i--)
                if (a[i] != p.a[i]) return (a[i] > p.a[i]) * 2 - 1;
            return 0;
        }
        ...
        if (tmp2.f == p.f) {
            ...
            if (com == 0) {
                ...
                for (int i = 0; i < t1->len; i++) {
                    ...
                    if (x + t1->a[i] < t2num) y = true, x += MOD;
    ...
    BigNumber operator*(const ll& _p) const {
        ll p = _p;
        if ((this->len == 1 && this->a[0] == 0) || !p) return BigNumber((lll)0);
        BigNumber tmp;
        tmp.f = !(f ^ (p > 0));
        if (p < 0) p = -p;
        tmp.len = 0;
        ...
        while (r) tmp.a[tmp.len++] = r % MOD, r /= MOD;
        return tmp;
    }

    BigNumber operator*(const BigNumber& p) const {
        if ((this->len == 1 && this->a[0] == 0) || (p.len == 1 && p.a[0] == 0)) return BigNumber((lll)0);
        BigNumber ret;
        ret.f   = !(f ^ p.f);
        ret.len = len + p.len + 5;
    ...
    BigNumber operator/(const ll& _p) const {
        ...
        for (int i = len - 1; ~i; --i) r = r * MOD + a[i], tmp.a[i] = r / p, r %= p;
        while (tmp.len > 1 && tmp.a[tmp.len - 1] == 0) --tmp.len;
        return tmp;
    }
    ...
    ll operator%(const ll& _p) const {
        ...
        if (p < 0) p = -p;
        lll r = 0;
        for (int i = len - 1; ~i; --i) r = r * MOD + a[i], r %= p;
        return is_pos ? r : -r;
    }
    ...
    BigNumber right_shift(int n) const {
        if (!n) return (*this);
        if (len <= n) return BigNumber((lll)0);
        BigNumber ret;
        ret.a   = new int[len - n + 2];
        ret.len = len - n, ret.f = f;
    ...
    BigNumber root(int k) const {
        assert(!((!this->f) && (!(k & 1))));
        if (this->is_zero()) return BigNumber((lll)0);
        BigNumber abs_x = abs(*this), one = BigNumber((lll)1);
        int       k1 = k - 1;
        BigNumber x = one.left_shift(abs_x.len / k + 1), xx = BigNumber((lll)0);
        while (1) {
            xx = abs_x / (x ^ k1);
            if (x <= xx) break;
            x = ((x * k1) + xx) / k;
    ...
    */
    ...
    struct bignum37 {
        ...
        bignum37(const char* s) {
            ...
            while (1) {
                ...
                i += bit_width;
                for (int j      = 0; j < bit_width;
                     a[len - 1] = (a[len - 1] << 3) + (a[len - 1] << 1) + (s[n - i + j] ^ 48), ++j)
                    ;
            }
        }
        inline BigNumber print() {
            ...
            while (x) ch[print_len++] = (x % base_radix) ^ '0', x /= base_radix;
            ...
            for (int i = len - 2; i >= 0; i--) {
                ...
                while (x) ch[print_len + k] = (x % base_radix) ^ '0', x /= base_radix, k--;

                while (k >= 0) ch[print_len + k] = '0', k--;

                print_len += bit_width;
            }

            if (print_len == 0) ch[print_len++] = '0';
        ...
        inline void left_shift() {
            ...
            if (a[len - 1] >= base) a[len] = a[len - 1] / base, a[len - 1] %= base, ++len;
        }
    ...
    bignum37 trans_to_37() const {
        ...
        int   print_len = 0, is_neg = 0;
        if (!f && (len > 1 || a[0] != 0)) ch[print_len++] = '-', is_neg = 1;
        int x = a[len - 1];
        while (x) ch[print_len++] = x % base_radix + '0', x /= base_radix;
        for (int i = is_neg; i < print_len + is_neg - 1 - i; i++)
            ch[i] ^= ch[print_len + is_neg - 1 - i], ch[print_len + is_neg - 1 - i] ^= ch[i],
    ...
    friend BigNumber gcd(BigNumber A, BigNumber B) {
        ...
        int      cnt = 0, tmp = 0;
        while (!(a.a[0] & 1) && !(b.a[0] & 1)) a.right_shift(), b.right_shift(), ++cnt;
        while (1) {
            while (!(a.a[0] & 1)) a.right_shift();

            while (!(b.a[0] & 1)) b.right_shift();
    ...
    void output() const {
        ...
        int x = a[len - 1];
        while (x) ch[print_len++] = (x % base_radix) ^ '0', x /= base_radix;
        for (int i = is_neg; i < print_len + is_neg - 1 - i; i++) std::swap(ch[i], ch[print_len + is_neg - 1 - i]);
        // ch[i] ^= ch[print_len + is_neg - 1 - i], ch[print_len + is_neg - 1 - i] ^= ch[i], ch[i] ^= ch[print_len +
        // is_neg - 1 - i];
        for (int i = len - 2; i >= 0; i--) {
            x     = a[i];
            int k = MODD - 1;
