...
??? node "Code by badcw"
    ...
    *
    ...
    int p[maxn << 2], tag[maxn << 2], staticx[maxn << 2];

    void build(int rt, int l, int r) {
        tag[rt] = 0;
        if (l == r) {
            staticx[rt] = p[rt] = dfn[l];
            return;
        }
        int mid = l + r >> 1;
        build(rt << 1, l, mid);
        build(rt << 1 | 1, mid + 1, r);
        if (dep[p[rt << 1]] > dep[p[rt << 1 | 1]]) staticx[rt] = p[rt] = p[rt << 1];
        else staticx[rt] = p[rt] = p[rt << 1 | 1];
    }
    ...
    void update(int rt, int l, int r, int le, int re, int k) {
        ...
        if (dep[p[rt << 1]] > dep[p[rt << 1 | 1]]) p[rt] = p[rt << 1];
        else p[rt] = p[rt << 1 | 1];
    }
    ...
    int deal(int x) {
        ...
        while (p[1]) {
            stk[++sz] = getfa(p[1], x);
            update(1, 1, n, st[stk[sz]], ed[stk[sz]], 0);
        }
        for (int i = sz; i >= 1; --i) update(1, 1, n, st[stk[i]], ed[stk[i]], 1);
        return sz;
    }
